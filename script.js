...
let LOGO_DESIRED_WIDTH_MM = 40; // Largura desejada da logo no PDF em mm
        ...
        try {
            ...
            try {
                ...
                h1, h2, .final-title, .diagnostico-title { 
                    ...
                    font-size: 14px !important; /* Títulos 14px */
                    font-weight: bold !important; 
                    margin-bottom: 3px !important; 
                    margin-top: 8px !important; 
                    letter-spacing: normal !important; 
                    word-spacing: normal !important; 
                    white-space: normal !important; /* Garantir espaçamento correto */
                } 
                h3, h4 { font-size: 11px !important; margin-top: 6px !important; margin-bottom: 2px !important; font-weight: bold !important; } /* Subtítulos 11px */
                ...
                * { margin-top: 0 !important; margin-bottom: 0 !important; padding-top: 0 !important; padding-bottom: 0 !important; }
                ...
                /* Regras específicas para garantir espaçamento em títulos */
                body * { letter-spacing: normal !important; word-spacing: normal !important; white-space: normal !important; }
    ...
    function displayFinalResults() {
        ...
        if (pdfBtn) {
            ...
            clone.addEventListener('click', function (ev) {
                ...
                // generatePdfAndSaveData (); COMENTADA EM 22/09 - INCLUSAO NOME E EMAIL NO PDF
            });
        }
    }
...
(function () {
    ...
    btn.addEventListener('click', async function (ev) {
        ...
        setTimeout(async () => {
            try {
                if (typeof generatePdfAndSaveData  === 'function') {
                    // await generatePdfAndSaveData (); ==> ALTERADO EM 25/09 PARA MUDAR A GERAÇÃO DO PDF COM LOGO E ENVIO PRA PLANILHA
                    await generatePdfAndSaveData();
                } else {
                    const finalEl = document.getElementById('final-resultado');
                    if (finalEl && typeof generatePdfAndSaveData === 'function') {
                        const blob = await generatePdfBlobFromElement(finalEl);
                        ...
                        if (typeof generatePdfAndSaveData  === 'function') {
                            generatePdfAndSaveData (document.getElementById('final-resultado'));
                        } else {
...
function exampleGeneratePdfAfterDelay(delay = 3000) {
    ...
    async function generatePdfAndSaveData() {
        ...
        if (finalResultEl) {
            ...
            // começar o html2canvas em uma nova página. Mas o objetivo aqui é TUDO na 1a página.
            // Vamos forçar a tentar encaixar.

            await html2canvas(finalResultEl, {
                scale: 1.1, // ESCALA para caber em uma página e reduzir tamanho do arquivo
                useCORS: true,
                logging: false,
                backgroundColor: '#ffffff',
                onclone: (clonedDoc) => customHtml2CanvasOnClone(clonedDoc, finalResultEl)
            }).then(canvas => {
                // Usando JPEG com qualidade 0.6 para reduzir ainda mais o tamanho do arquivo
                const imgData = canvas.toDataURL('image/jpeg', 0.6);
                const contentImgWidth = 210 - (pdfContentMarginX * 2); // Largura da imagem no PDF: A4 - (2 * Margem)
                const contentImgHeight = canvas.height * contentImgWidth / canvas.width; // Calcula altura proporcional
        ...
        doc.save('Diagnostico_Estilo.pdf');
                    ...
                    try {
                        ...
                        try {
                            ...
                            safetyStyle.textContent = `
                                ...
                                h1, h2, .final-title, .diagnostico-title { text-align: center !important; font-size: 18px !important; font-weight: 600 !important; margin-bottom: 12px !important; }
                            ...
                            try {
                                ...
                                if (possibleTitle) {
                                    ...
                                    possibleTitle.style.fontSize = '18px';
